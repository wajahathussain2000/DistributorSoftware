...
namespace DistributionSoftware.Business
{
    public interface ISalesmanTargetService
    {
        ...
        SalesmanTarget CalculateTargetPerformance(SalesmanTarget target);
        SalesmanTargetAchievement CalculateAchievementPerformance(SalesmanTargetAchievement achievement, SalesmanTarget target);
        List<SalesmanTarget> GetSalesmanPerformanceSummary(int salesmanId, DateTime startDate, DateTime endDate);
        List<SalesmanTarget> GetTeamPerformanceSummary(DateTime startDate, DateTime endDate);
        bool UpdateTargetAchievement(int targetId);
        bool GenerateMonthlyTargets(int salesmanId, DateTime month);
        bool GenerateQuarterlyTargets(int salesmanId, DateTime quarter);
        bool GenerateYearlyTargets(int salesmanId, DateTime year);
        List<SalesmanTarget> GetTargetsDueForReview();
        List<SalesmanTarget> GetTargetsNearDeadline(int daysBeforeDeadline);
        bool SendTargetReminder(int targetId);
        bool SendAchievementReminder(int achievementId);
    }
}
