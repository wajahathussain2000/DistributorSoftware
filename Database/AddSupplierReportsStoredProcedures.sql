...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN st.Amount > 0 AND st.TransactionType IN ('Purchase', 'Debit Note') THEN 'Outstanding'
            WHEN st.Amount < 0 AND st.TransactionType IN ('Payment', 'Return') THEN 'Paid'
            ELSE 'Balanced'
...
BEGIN
    ...
    WITH SupplierBalances AS (
        SELECT 
            ...
            ISNULL(SUM(CASE 
                WHEN st.TransactionType IN ('Purchase', 'Debit Note') THEN st.Amount 
                ELSE 0 
            END), 0) AS TotalDebits,
            ISNULL(SUM(CASE 
                WHEN st.TransactionType IN ('Payment', 'Return', 'Credit Note') THEN ABS(st.Amount) 
                ELSE 0 
...
BEGIN
    ...
    SELECT 
        ...
        (SELECT ISNULL(SUM(CASE 
            WHEN st2.TransactionType IN ('Purchase', 'Debit Note') THEN st2.Amount 
            ELSE 0 
        END), 0) - ISNULL(SUM(CASE 
            WHEN st2.TransactionType IN ('Payment', 'Return', 'Credit Note') THEN ABS(st2.Amount) 
            ELSE 0 
        END), 0)
    ...
    LEFT JOIN SupplierTransactions st ON sp.SupplierId = st.SupplierId 
        AND st.ReferenceNumber = sp.ReferenceNumber
