...
-- Create date: 2025
-- Description: Fixed stored procedure to get expense report data
...
BEGIN
    ...
    SELECT 
        ...
        e.UserId,
        'User ' + CAST(e.UserId AS VARCHAR(10)) AS UserName,
        'user' + CAST(e.UserId AS VARCHAR(10)) AS Username,
        'user' + CAST(e.UserId AS VARCHAR(10)) + '@company.com' AS UserEmail,
        '0300-' + RIGHT('0000000' + CAST(e.UserId AS VARCHAR(10)), 7) AS UserPhone,
        e.PaymentMethod,
        e.ReferenceNumber,
        e.ReceiptNumber,
        e.VendorName,
        ...
        e.Status,
        e.ApprovedBy,
        CASE WHEN e.ApprovedBy IS NOT NULL THEN 'User ' + CAST(e.ApprovedBy AS VARCHAR(10)) ELSE NULL END AS ApprovedByName,
        e.ApprovedDate,
        e.Remarks,
        e.CreatedBy,
        'User ' + CAST(e.CreatedBy AS VARCHAR(10)) AS CreatedByName,
        e.CreatedDate,
        e.ModifiedBy,
        CASE WHEN e.ModifiedBy IS NOT NULL THEN 'User ' + CAST(e.ModifiedBy AS VARCHAR(10)) ELSE NULL END AS ModifiedByName,
        e.ModifiedDate,
